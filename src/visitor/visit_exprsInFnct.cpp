...
void 
visit_exprsInFnct::clearData(void) {
    tempS = "";
    name = "";
    type = "";
    typeExtra = "";
...
// visit declarations
void
visit_exprsInFnct::visit_var_decl (var_decl_n *np)
{
  nast_node      *child;
  varUsedData*   vuData;
...
void
visit_exprsInFnct::visit_parm_decl (parm_decl_n *np)
{
  nast_node      *child;
  varUsedData*   vuData;
...
// visit types
void
visit_exprsInFnct::visit_void_type (void_type_n *np)
{
  type = "void ";
...
void
visit_exprsInFnct::visit_integer_type (integer_type_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_enumeral_type (enumeral_type_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_real_type (real_type_n *np)
{
...
void
visit_exprsInFnct::visit_array_type (array_type_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_record_type (record_type_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_union_type (union_type_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_pointer_type (pointer_type_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_function_type (function_type_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_othr_type (othr_type_n *np)
{
  nast_node *child;
...
// visit statements
void
visit_exprsInFnct::visit_break_stmt (break_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_continue_stmt (continue_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_scope_stmt (scope_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_decl_stmt (decl_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_expr_stmt (expr_stmt_n * np)
{
  nast_node *child;

  cout << "Visiting  ExprStmt ... \n";
  ...
  if (np->hasEdgeName(EXPRESSION_EDGE)) {
    ...
    // Fill information
    eData->setLine(np->getLine());
    eData->setPosName(EXPR_STMT);
    eData->setExprStat(exprStat);
    eData->setVarUsed(vuL);
    
    exprL->push_back(eData);
  }
...
void
visit_exprsInFnct::visit_goto_stmt (goto_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_label_stmt (label_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_return_stmt (return_stmt_n * np)
{
  nast_node *child;

  cout << "Visiting  return ... \n";
...
void
visit_exprsInFnct::visit_if_stmt (if_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_switch_stmt (switch_stmt_n * np)
{
  nast_node *child;

  cout << "Visiting  switch ... \n";
...
void
visit_exprsInFnct::visit_for_stmt (for_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_while_stmt (while_stmt_n * np)
{
  nast_node *child;
  ...
  if (np->hasEdgeName(CONDITION_EDGE)) {
    ...
    cout << "vuL is empty ? " << vuL->empty() << "\n";

    exprL->push_back(eData);

    cout << "exprL size ? " << exprL->size() << "\n";
  }
...
void
visit_exprsInFnct::visit_do_stmt (do_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_compound_stmt (compound_stmt_n * np)
{
  nast_node *child;

  cout << "Visiting compound ... \n";
...
void
visit_exprsInFnct::visit_asm_stmt (asm_stmt_n * np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_othr_stmt (othr_stmt_n * np)
{
  nast_node *child;
...
// visit expressions
void
visit_exprsInFnct::visit_unary_expr (unary_expr_n *np)
{
  nast_node *child;
  
  exprStat->addExpr(np->getCode());
...
void
visit_exprsInFnct::visit_binary_expr (binary_expr_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_ternary_expr (ternary_expr_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_ref_expr (ref_expr_n *np)
{
  nast_node *child;
...
void
visit_exprsInFnct::visit_othr_expr (othr_expr_n *np)
{
  nast_node *child;
...
// visit other nodes
void
visit_exprsInFnct::visit_identifier (identifier_n *np)
{
  tempS = np->getString();
  return;
}
...
void
visit_exprsInFnct::visit_tree_list (tree_list_n *np)
{
  /* This method is used :
     ...
     attribute of the class visit_exprsInFnct are setted and after
     are used by the caller of this method.
     When it build an argument the the local variable and the 
     attribute of the class visit_exprsInFnct are setted
     but they aren't used by the caller of this method.

     Remember : the parm_decl and var_decl clear the attribute of the
     class visit_exprsInFnct, so it don't use any illegal 
...
void 
visit_exprsInFnct::visit_case_label(case_label_n *np) 
{
  nast_node *child;
...
void 
visit_exprsInFnct::init(void) {
  declStmtL.clear();
}

//! The finalization operation
void 
visit_exprsInFnct::finish(void) {

  exprsListIterator         pExprL;
  varUsedListIterator       pVUL;
  exprData                  *eData;
  varUsedData               *vuData;

  for (pExprL = exprL->begin(); pExprL != exprL->end(); pExprL++) {
    eData = *pExprL;
    vuL = eData->getVarUsed();

    // Mark the global or local scope
    // read variables used, one for time 
    for (pVUL = vuL->begin(); pVUL != vuL->end();  pVUL++) {
